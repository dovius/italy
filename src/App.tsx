...
function TripApp({ trip, setTrip, storageError }: { trip: SavedTrip; setTrip: Dispatch<SetStateAction<SavedTrip>>; storageError: boolean }) {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content" onClick={(event) => { event.preventDefault(); const main = document.getElementById('main-content'); if (main) { main.tabIndex = -1; main.focus(); main.scrollIntoView({ block: 'start' }); } }}>Pereiti prie turinio</a><header className="site-header"><button className="brand" onClick={() => navigate('home')} aria-label="Kelionės vertėjas – pradžia"><LogoMark /><span>Kelionės vertėjas<small>ITALIJA ARČIAU</small></span></button><div className="header-right"><span className={`network-status ${online ? '' : 'is-offline'}`}><span className="mini-dot" />{online ? 'Buon viaggio!' : 'Nėra ryšio'}</span><button className="help-button" aria-label="Kaip naudotis?" onClick={() => setHelp(true)}><CircleHelp size={21} /><span>Kaip naudotis?</span></button></div></header>
    ...
    {help && <Modal title="Viskas labai paprasta." onClose={() => setHelp(false)}><p className="modal-lead">Trys būdai jaustis drąsiau Italijoje.</p><div className="help-step"><span className="help-step-icon terracotta"><Mic size={25} /></span><div><h3>Kalbėtis</h3><p>Paspauskite ir leiskite naudoti mikrofoną. Kalbėkite lietuviškai, pašnekovas – savo kalba. Vertimą abu išgirsite balsu.</p></div></div><div className="help-step"><span className="help-step-icon sand"><Camera size={25} /></span><div><h3>Fotografuoti</h3><p>Nufotografuokite tekstą arba įkelkite nuotrauką. Išversime ir paaiškinsime. Apie tą pačią nuotrauką galite klausti toliau.</p></div></div><div className="help-step"><span className="help-step-icon sage"><MessageCircle size={25} /></span><div><h3>Paklausti</h3><p>Parašykite arba padiktuokite klausimą lietuviškai. Padėsime susigaudyti kelionėje.</p></div></div><div className="help-detail"><h3>Ryšys ir privatumas</h3><p>Naujiems vertimams reikia interneto. Pokalbio tekstas ir paskutinė nuotrauka išsaugomi šiame telefone.</p><p>Vertimui garsas, nuotraukos ir klausimai siunčiami „OpenAI“. Garso įrašų mūsų serveris nesaugo. Balso pokalbis baigiamas išėjus iš jo ekrano.</p></div><button className="button primary full-width" onClick={() => setHelp(false)}><Check size={22} /> Supratau</button><button className="clear-data" onClick={() => setReset('all')}>Ištrinti šiame telefone išsaugotą pokalbį ir nuotrauką</button></Modal>}
