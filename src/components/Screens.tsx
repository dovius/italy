...
export function LiveScreen({ status, error, level, rows, muted, blocked, elapsed, onBack, start, end, play, speak, show }: {
  ...
  return <main className="tool-page live-page" id="main-content"><ScreenHeader eyebrow="GYVAS VERTĖJAS" title="Susikalbėkime." description="Jūs kalbate. Mes išverčiame balsu." onBack={onBack} /><div className="language-strip"><span><span className="lithuanian-flag" aria-hidden="true" /> Lietuvių</span><span className="language-arrows" aria-hidden="true">⇄</span><span><span className="italian-flag" aria-hidden="true" /> Italų</span></div><p className="language-note">Taip pat suprantame anglų ir ispanų kalbas.</p>
    {error && <Notice retry={!active ? start : undefined}>{error}</Notice>}
    <section className={`live-stage ${status === 'connected' && !muted ? 'is-live' : ''}`} aria-label="Pokalbio būsena"><div className="live-stage-top"><span className={`connection-label ${status === 'connected' ? 'connected' : ''}`}><span className="mini-dot" />{status === 'connected' ? 'POKALBIS VYKSTA' : status === 'connecting' ? 'JUNGIAMĖS' : status === 'reconnecting' ? 'ATKURIAME RYŠĮ' : status === 'ended' ? 'POKALBIS BAIGTAS' : 'PASIRUOŠĘ KALBĖTIS'}</span>{active && <span className="call-clock">{clock}</span>}</div>
      <div className="live-orb">{status === 'connecting' || status === 'reconnecting' ? <LoaderCircle className="spin" size={38} /> : muted ? <MicOff size={38} strokeWidth={1.5} /> : <Mic size={38} strokeWidth={1.5} />}</div>
      <h2 aria-live="polite">{muted && active ? 'Mikrofonas pristabdytas' : status === 'connected' ? 'Klausomės. Galite kalbėti.' : status === 'connecting' ? 'Tuoj galėsime kalbėtis…' : status === 'reconnecting' ? 'Bandome prisijungti…' : status === 'ended' ? 'Ačiū už pokalbį.' : 'Kalbėkite savo kalba.'}</h2>
      <p>{active ? 'Laikykite telefoną tarp savęs ir pašnekovo.' : 'Vienas paspaudimas – ir vertėjas klausosi.'}</p>
    ...
    {rows.length > 0 ? <section className="transcript-section"><div className="section-intro"><h2>Pokalbio tekstas</h2><span>Išgirsta ir išversta</span></div><Captions rows={rows} />{last && <div className="translation-actions"><button className="button secondary" onClick={() => speak(last.text)}><Volume2 size={23} /> Pakartoti</button><button className="button secondary" onClick={() => show(last.text)}><Eye size={23} /> Parodyti žmogui</button></div>}</section> : <div className="empty-transcript"><MessageCircle size={24} /><p>Čia matysite, kas pasakyta ir kaip išversta.</p></div>}
...
export function AssistantScreen({ messages, chat, draft, setDraft, onBack, speak, clear }: { messages: Message[]; chat: ChatController; draft: string; setDraft: (value: string) => void; onBack: () => void; speak: (text: string) => void; clear: () => void }) {
  return <main className="tool-page assistant-page" id="main-content"><ScreenHeader eyebrow="KELIONĖS ASISTENTAS" title="Drąsiai klauskite." description="Apie maistą, transportą ir visa kita Italijoje." onBack={onBack} />
    ...
    {!messages.length && <section className="assistant-suggestions"><div className="suggested-label">GALITE PRADĖTI NUO ŠIŲ KLAUSIMŲ</div><div className="question-grid">{questions.map(({ icon: Icon, text }) => <button key={text} onClick={() => chat.send(text)}><Icon size={23} /><span>{text}</span><ArrowRight size={19} /></button>)}</div></section>}
