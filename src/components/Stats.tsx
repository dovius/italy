...
export default function Stats() {
  ...
  function expired() { setAccess('login'); setData(null); setDetail(null); setEditing(null); }
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!controller.signal.aborted) { setData(result); setAccess('ready'); setUpdated(Date.now()); setError(''); initial.current = false; }
      } catch (cause) {
  ...
  }, [kind, visitor, conversation, query, from, to, page, refresh]);
